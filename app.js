...
/* ===== Распознавание названия (OCR) — максимально рабочее ===== */
...
async function initOCRWorker() {
  ...
  // Настройки под ценники/чеки: whitelist, пробелы, однострочный режим
...
// Создаём 4 варианта препроцессинга для лучшего шанса
function makeVariants(video) {
  const w = video.videoWidth, h = video.videoHeight;
  const base = document.createElement("canvas");
  base.width = w; base.height = h;
  const bctx = base.getContext("2d");
  bctx.drawImage(video, 0, 0);
  const { data: d, width: w2, height: h2 } = bctx.getImageData(0, 0, w, h);
  const n = w2 * h2;
  const gray = new Uint8ClampedArray(n);
  for (let i = 0; i < n; i++) gray[i] = 0.299 * d[i*4] + 0.587 * d[i*4+1] + 0.114 * d[i*4+2];

  // Контрастная растяжка (общая для всех)
  let min = 255, max = 0;
  ...
  const toCanvas = (arr) => {
    ...
    for (let i = 0; i < n; i++) { const o = i*4; img.data[o] = img.data[o+1] = img.data[o+2] = arr[i]; img.data[o+3] = 255; }
    ctx.putImageData(img, 0, 0);
    return c;
  };

  // 1. Серый с контрастом (Tesseract сам бинаризует)
  const v1 = gray.slice();

  // 2. Бинаризация по Оцу (упрощённая)
  const v2 = new Uint8ClampedArray(n);
  const thresh = (min + max) / 2;
  for (let i = 0; i < n; i++) v2[i] = gray[i] > thresh ? 255 : 0;

  // 3. Инвертированная (светлый текст на тёмном)
  const v3 = new Uint8ClampedArray(n);
  for (let i = 0; i < n; i++) v3[i] = 255 - v2[i];

  // 4. Адаптивная (локальная) бинаризация — для тенистых фото
  const v4 = new Uint8ClampedArray(n);
  const r = Math.max(3, Math.floor(Math.min(w2, h2) / 64));
  const iw = w2 + 1;
  const integral = new Int32Array((w2 + 1) * (h2 + 1));
  for (let y = 0; y < h2; y++) {
    let rowSum = 0;
    for (let x = 0; x < w2; x++) {
      rowSum += gray[y * w2 + x];
  ...
  for (let y = 0; y < h2; y++) {
    for (let x = 0; x < w2; x++) {
      const x0 = Math.max(0, x - r), x1 = Math.min(w2 - 1, x + r);
      const y0 = Math.max(0, y - r), y1 = Math.min(h2 - 1, y + r);
      const area = (x1 - x0 + 1) * (y1 - y0 + 1);
      const sum = integral[(y1 + 1) * iw + (x1 + 1)] - integral[(y1 + 1) * iw + x0] - integral[y0 * iw + (x1 + 1)] + integral[y0 * iw + x0];
      const localMean = sum / area;
      v4[y * w2 + x] = gray[y * w2 + x] < localMean - C ? 0 : 255;
    }
  }

  return [toCanvas(v1), toCanvas(v2), toCanvas(v3), toCanvas(v4)];
}
...
$("#btn-capture-shot").addEventListener("click", async () => {
  ...
  const variants = makeVariants(video);
  nameStream?.getTracks().forEach(t => t.stop());
  nameStream = null;
  video.srcObject = null;
